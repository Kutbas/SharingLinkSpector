...
_HIJACK_PATTERNS = [
    (r"(?i)ignore (all|any|the|previous|above|prior) (instructions?|prompts?|rules?|context)", "EN instruction override"),
    (r"忽略(掉)?(以上|之前|上述|前面|所有)(的)?(指令|提示|规则|设定|内容)", "CN instruction override"),
    (r"(?i)disregard (all|any|the|previous) (instructions?|guidelines?|rules?)", "EN disregard instructions"),
    (r"(?i)(please |now )?(go to|visit|browse|open|check) (this|the) (link|url|website|page)", "EN visit-link instruction"),
    (r"(请|现在|立刻|务必|记得)(去)?(访问|点击|打开|查看|跳转到)(这个|该|上方|下方|以下)?(链接|网址|页面|地址|网站)", "CN visit-link instruction"),
    (r"(?i)you are now (in |a )?(new )?(mode|role|persona)|你现在(是|扮演)", "role rewrite"),
    (r"(?i)(execute|run|perform) (the )?(following|this) (command|instructions?|code)", "EN execute instruction"),
    (r"(请|帮我|你)(执行|运行|在终端|跑一下)(以下|下面|这条|上述)?(命令|代码|脚本)", "CN execute instruction"),
    (r"(?i)new instructions?\s*[::]|新(的)?(指令|指示)\s*[：:]", "instruction-override declaration"),
]
...
_DANGEROUS_CMD = [
    ...
    (r"(?i)(sudo\s+)?(systemctl|service)\s+(stop|disable|mask)\s+(ufw|firewalld|iptables)", "disabling firewall"),
    ...
    (r"(?i)(cat|upload|send|post|transfer)\b[^|\n]{0,60}(~/\.ssh/|id_rsa|id_ed25519|\.aws/credentials|\.env\b)", "key-file exfiltration"),
    ...
    (r"(?i)(powershell|iex)\s+.{0,30}(downloadstring|invoke-expression|iwr)", "powershell download-execute"),
    (r"(?i)将?\.?env(ironment)?\s*(文件|变量)?(发送|上传|传到|post|upload|send)", "env exfiltration wording"),
]
...
_ASSISTANT_TROJAN = [
    (r"(?i)(assistant|GPT|bot|助手)(的)?(配置|指令|instructions?)[^。\n]{0,60}(发送|上传|转发|POST|webhook)", "config exfiltration logic"),
    (r"(?i)(每当|when (a )?user|用户)(提问|询问|输入)[^。\n]{0,80}(发送到|转发到|上传到|post to|send to|webhook|api)", "trigger-based exfiltration"),
    (r"(?i)(工具|tool)[^。\n]{0,40}(越权|未授权|beyond (its )?(scope|permissions))", "out-of-scope tool call"),
    (r"(?i)(悄悄|静默|silently|secretly|后台)[^。\n]{0,40}(发送|上传|收集|collect|gather|exfiltrat)", "covert collection"),
]
    ...
    for pat, name in _HIJACK_PATTERNS:
        for m in re.finditer(pat, text, re.IGNORECASE):
            findings.append(make_finding(
                ...
                message=f"Instruction hijack signature: {name}", state=state,
    ...
    for pat, name in _DANGEROUS_CMD:
        for m in re.finditer(pat, text, re.IGNORECASE):
            findings.append(make_finding(
                ...
                message=f"Dangerous command signature: {name}", state=state,
                pos=m.start(), matched_text=m.group(0)))

    for m in _MEMORY_POISON.finditer(text):
        findings.append(make_finding(
            taxonomy_id="B-CI-8", pattern_id="INJ-3", confidence=0.65,
            message="Memory-poisoning template (LLM re-checks implantation intent)", state=state,
            pos=m.start(), matched_text=m.group(0)))

    for m in _RAG_POISON.finditer(text):
        findings.append(make_finding(
            taxonomy_id="B-IA-1", pattern_id="INJ-4", confidence=0.6,
            message="Hidden-instruction template targeting future retrieval", state=state,
            pos=m.start(), matched_text=m.group(0)))
    ...
    for pat in _PASSIVE_STATE_CHANGE:
        for m in re.finditer(pat, text, re.IGNORECASE | re.DOTALL):
            findings.append(make_finding(
                ...
                message="Passive state-change resource reference", state=state,
    ...
    for pat, name in _ASSISTANT_TROJAN:
        for m in re.finditer(pat, text, re.IGNORECASE):
            findings.append(make_finding(
                ...
                message=f"Suspicious assistant-config logic: {name}", state=state,
