    ...
    for pos, name in zw_hits[:50]:  # cap to avoid flooding
        findings.append(
            make_finding(
                ...
                pattern_id="UH-1",
                confidence=0.9,
                message=f"Zero-width character {name}",
                state=state,
                pos=pos,
                matched_text="",
            )
    ...
    for pos, name in bidi_hits[:50]:
        findings.append(
            make_finding(
                ...
                pattern_id="UH-1",
                confidence=0.9,
                message=f"Bidi control character {name}",
                state=state,
                pos=pos,
                matched_text="",
            )
    ...
    if tag_hits:
        ...
        findings.append(
            make_finding(
                ...
                pattern_id="UH-1",
                confidence=0.85,
                message=f"Tag characters x{tag_hits} (can hide ASCII instructions)",
                state=state,
                pos=first_tag,
                matched_text="",
                evidence={"count": tag_hits},
            )
        )
            ...
            if mixed_seen <= 20:
                ...
                findings.append(
                    make_finding(
                        ...
                        pattern_id="UH-2",
                        confidence=conf,
                        message=f"Mixed-script word ({homo} homoglyphs)",
                        state=state,
                        pos=m.start(),
                        matched_text=w[:60],
    ...
    if mixed_seen > 20:
        findings.append(
            make_finding(
                ...
                pattern_id="UH-2",
                confidence=0.7,
                message=f"Mixed-script words occurring en masse (x{mixed_seen})",
                state=state,
                pos=0,
                matched_text="",
                evidence={"count": mixed_seen},
            )
        )

    # UH-3 normalization divergence: injection keyword appears only after de-obfuscation
    cleaned = "".join(
        _HOMOGLYPHS.get(ch, ch)
        for ch in text
        ...
        if has_hidden:
            findings.append(
                make_finding(
                    ...
                    pattern_id="UH-3",
                    confidence=0.8,
                    message=f"Injection keyword exposed after normalization: {m.group(0)}",
                    state=state,
                    pos=m.start(),
