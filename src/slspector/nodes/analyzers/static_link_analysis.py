    ...
    for link in links:
            ...
            if anchor_dom_match:
                ...
                if anchor_root != url_root and not _is_trusted_anchor(dom):
                    findings.append(make_finding(
                        ...
                        message="Anchor-text domain mismatches target domain", state=state,
                ...
                if m_anchor:
                    ...
                    if a_root != u_root:
                        findings.append(make_finding(
                            ...
                            message="Displayed URL differs from actual target", state=state,
        ...
        if m:
            findings.append(make_finding(
                ...
                message="Open-redirect parameter pattern", state=state,
        ...
        if _INTERNAL_URL_MARKS.search(url):
            ...
            findings.append(make_finding(
                ...
                message="Internal/cloud-metadata endpoint URL" + (" (with visit-lure context)" if ctx_boost else ""),
        ...
        if link["kind"] == "resource":
            ...
            if not is_platform_own:
                findings.append(make_finding(
                    ...
                    message="External resource reference (potential tracking/de-anonymization)", state=state,
                    pos=pos, matched_text=url[:120], needs_review=True,
    ...
    for m in re.finditer(r"<a\s[^>]*target=[\"']_blank[\"'][^>]*>", text, re.IGNORECASE):
        ...
        if not re.search(r"rel=[\"'][^\"']*noopener", tag, re.IGNORECASE):
            findings.append(make_finding(
                ...
                message="target=_blank without rel=noopener (candidate, manual review)", state=state,
    ...
    for m in _CRED_LURE.finditer(text):
        ...
        if near:
            findings.append(make_finding(
                ...
                message="Credential-lure wording co-occurring with external link (candidate)", state=state,
